...
![](../.gitbook/assets/txt1.jpg)

`dp` 数组指示 `pat` 这样移动：

![](../.gitbook/assets/txt2.jpg)

PS：这个`j` 不要理解为索引，它的含义更准确地说应该是**状态**（state），所以它会出现这个奇怪的位置，后文会详述。

而对于 `txt2` 的下面这个即将出现的未匹配情况：

![](../.gitbook/assets/txt3.jpg)

`dp` 数组指示 `pat` 这样移动：

![](../.gitbook/assets/txt4.jpg)
